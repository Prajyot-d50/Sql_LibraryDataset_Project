...
select branch_no,sum(Salary) from Employee
...
from Customer A
join Return_Status B
on A.Customer_Id=B.Return_cust
join Cte_books C
on B.isbn_book2=C.ISBN
...
delete from #lb_books
where Status='No'

select *
into books_avalible_books
from #lb_books
...
create procedure Book_D @Category varchar(50) as
begin
select sum(Rental_Price) as Total_Price
from Books
where Category = @Category

select count(Status) as Status_
from Books
where Status ='Yes' and Category= @Category
end

exec Book_D @Category = 'Classic'

select * from Books


alter procedure Salary_D  @Position varchar(50)
as
begin

select max(salary) as second_salary,Position
from Employee
where Position = @Position and
Salary < (select max(Salary) from Employee  where Position= @Position)
group by Position
